<script lang="ts">
	...
	import { BadgeWithIcon, CardEnhanced } from '$lib/components/ui';
...
<CardEnhanced margin="0px 0px 20px 0px" tiltDegree={2} href={`${base}/experience/${experience.slug}`}>
	<div class="col md:flex-row items-start gap-5 md:gap-1">
		...
		<div class="col ml-0 md:ml-[20px] gap-3 w-full">
			...
			<div class="row flex-wrap m-b-2 gap-1 text-0.9em font-300">
				<BadgeWithIcon name={`Company: ${experience.company}`}>
					<div><UIcon icon="i-carbon-building" /> {experience.company}</div>
				</BadgeWithIcon>
				<BadgeWithIcon name={`Location: ${experience.location}`}>
					<UIcon icon="i-carbon-location" />
				</BadgeWithIcon>
				<BadgeWithIcon name={`Contract: ${experience.contract}`}>
